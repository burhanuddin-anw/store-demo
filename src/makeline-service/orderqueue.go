...
func getOrdersFromQueue() ([]Order, error) {
	...
	ctx, span := otel.Tracer("makeline-service").Start(
		context.Background(),
		"getOrdersFromQueue",
		trace.WithAttributes(attribute.String("service", "makeline-service")),
	)
	defer span.End()
	...
	if orderQueueName == "" {
		...
		span.RecordError(errors.New("ORDER_QUEUE_NAME is not set"))
